...
function TaskItem({
	...
	return (
		<div
			className='card'
			style={{
				backgroundColor: numberToColorTicket(urgenceLevel),
				border: `2px solid ${numberToColorTicketBorder(urgenceLevel)}`,
			}}
		>
			<h2>Niveau d&apos;urgence : {chipVariant(urgenceLevel)}</h2>
			<h3>{title}</h3>
			{status == 'résolu' ? (
				<p id='status'>Le ticket est résolu</p>
			) : (
				<p id='status'>Le ticket est actuellement en cours</p>
			)}
