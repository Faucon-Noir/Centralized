        ...
        if (isTaskStarted && !isTaskComplete) {
            ...
            const checkTaskStatus = () => {
                intervalId = setInterval(async () => {
                    try {
                        let responseStatus = await axios.get(`http://localhost:8000/api/specification/check-status`, {
        ...
        <>
            {showPopup && (
                <div className='reqLoad'>
                    ...
                    <p style={{ fontSize: "10px" }}>La page cahier des charges est temporairement désactivée</p>
                    <BarLoader
                        color="white"
                        loading={true}
                        speedMultiplier={1}
