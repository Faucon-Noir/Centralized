...
function CalendarBox({ name, start_date, end_date, description, color }: any) {
	...
	let formatted_start_date = start_date.toLocaleDateString('fr-FR', {
		day: '2-digit',
		month: '2-digit',
		year: '2-digit',
	});
	let formatted_end_date = end_date.toLocaleDateString('fr-FR', {
		day: '2-digit',
		month: '2-digit',
		year: '2-digit',
	});
	const tileClassName = ({ date, view }: any) => {
        ...
        let codeColor = ' color_' + color.substring(1);
		...
		if (
			...
			if (
				(date.getDay() === 0 && dateString === startDateString) ||
				...
				<div className='calendarBox'>
					...
					<div className='texte_calendar'>
						<div className='titre_calendar'>
							<h2>{name ? name : 'test'}</h2>
							<div
								className='circlemini'
								style={{
						...
						<div className='flex_align'>
							...
							<p>
								du {formatted_start_date} au{' '}
								{formatted_end_date}
							</p>
						</div>
						<div className='flex_align'>
							<InsertDriveFileOutlinedIcon />
							<p>{description ? description : 'test'}</p>
						</div>
					</div>
				</div>
			) : (
