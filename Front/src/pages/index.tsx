      ...
      useEffect(() => {
        ...
        try {
          axios.get(`http://localhost:8000/api/project/user/${user_id}`, {
            ...
            res.data.forEach((element: { id: any; name: string }) => {
              try {
              ...
              //GET REX INFO
              try {
                axios.get(`http://localhost:8000/api/rex/project/${element.id}`, {
                  headers: { Authorization: `Bearer ${token}` }
                }).then(res => {
                  if (!res.data.error) {
          ...
          <div className="right_container">
            ...
            <Grid container sx={{ display: 'flex', flexDirection: 'column', gap: '50px' }}>
              <div className="DeuxEtapes">
                <div className="calendar_container">
                  <CalendarBox name={lastproject ? lastproject.name : ''} start_date={lastproject ? new Date(lastproject.start_date) : new Date()} end_date={lastproject ? new Date(lastproject.end_date) : new Date()} description={lastproject ? lastproject.description : ''} />
                </div>

                <div className="DernierTicket">
                  <div className="entetedernierticket">
                    <h2>Derniers tickets</h2>
                    <ButtonBase><Image height={20} width={20} alt="" src="/assets/arrow-narrow-right.svg" /></ButtonBase>
                  </div>
                  {ticket.filter((task: any, idx: number) => idx < 3).map((task: any) => (
                    <TaskCard id={task.ticket_id} title={task.ticket_title} urgenceId={task.ticket_urgence} date={task.ticket_start_date} key={task.ticket_id} />
                  ))}
                </div>
              </div>
              <div className="DeuxEtapes second_line">
                <div className="DernierTicket">
                  ...
                  {specification.filter((value: any, idx: number) => idx < 3).map((value: any) => (
                    <SpecificationCard id={value.id} title={value.name} key={value.id} />
                  ))}
                </div>
                <div className="DernierTicket">
                  <h2>Retours d&apos;expériences</h2>
                  <RexCard answer1={lastrex ? lastrex.answer1 : ""} answer2={lastrex ? lastrex.answer2 : ""} answer3={lastrex ? lastrex.answer3 : ""} name={lastrex ? lastrex.name : ""} />
                </div>
              </div>
            </Grid>
            <div className="MyTeam">
              <div className='Entete'>
                <div className='TitleProjetCards'>
                  <h2>Mon équipe</h2>
                  <ButtonBase><AddIcon fontSize='medium' sx={{ color: "#000000" }} /></ButtonBase>
                </div>
                <div className="fleches">
                  <Image src="/assets/chevron-left.svg" alt="" width={20} height={20} className="swiper-button-prev swiper-button-prev-2 fleche"></Image>
                  <Image src="/assets/chevron-right.svg" alt="" width={20} height={20} className="swiper-button-next swiper-button-next-2 fleche"></Image>
                </div>
              </div>
