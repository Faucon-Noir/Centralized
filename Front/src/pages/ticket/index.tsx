...
import { ModalContentStyle, SaveButtonStyle, UnderTitleBoxStyle, inputStyle } from "./style";
...
export default function Tickets() {
  ...
  const statusOptions: string[] = ["ouvert", "en cours", "en attente", "annulé", "résolu", "rejeté", "en revue"];
  for (let i: number = 0; i < statusOptions.length; i++) {
    statusList.push(<option key={i} value={statusOptions[i]}>{statusOptions[i]}</option>);
  }
  ...
  const handleArchive = (id: string) => {
    ...
    axios.patch(`http://localhost:8000/api/ticket/${id}`, { status: "résolu" }, {
  ...
  const handleSave = () => {
    ...
    try {
      axios.patch(`http://localhost:8000/api/ticket/${selectedTaskId}`, ticketDetails, {
        headers: { Authorization: `Bearer ${token}` }
      }).then(res => {
        if (res.status === 200) {
          try {
        ...
        <Grid xs={9}>
          <div className="ticket_container">
            <div className="header" style={{ width: '100%', marginTop: "100px", marginLeft: '20px', }}>
              <h1>Mes tickets</h1>
              <Button href="/ticket/create"><AddIcon fontSize='medium' sx={{ color: "#000000" }} /></Button>
            ...
            <div className="card_container">
              {tickets.map((ticket: any) => (
                <>
                  <TaskItem id={ticket.ticket_id}
                    title={ticket.ticket_title}
                    urgenceLevel={ticket.ticket_urgenceId}
                    date={ticket.ticket_start_date}
                    status={ticket.ticket_status}
                    onOpen={() => handleOpen(ticket.ticket_id)}
                    onArchive={() => handleArchive(ticket.ticket_id)}
                    onRedirect={() => handleToSpecification(ticket.projectId)}
                  /></>
              ))}
            </div>
          </div>
        ...
        <Modal open={open}>
          <Box sx={ModalContentStyle}>
            ...
            <TextField
              ...
              onChange={(event) => setTicketDetails({ ...ticketDetails, title: event.target.value })}
              sx={{ margin: '20px' }}
            />
            <Box sx={UnderTitleBoxStyle}>
              {/* Niveau d'urgence du ticket */}
              {/* Liste déroulante pour le niveau d'urgence du ticket */}
              Urgence: <select style={inputStyle} defaultValue={selected?.ticket_urgenceId} onChange={(e) => setTicketDetails({ ...ticketDetails, urgenceId: parseInt(e.target.value) })}>
                {options}
              </select>
              {/* Status du ticket */}
              {/* Liste déroulante pour le statut */}
              Status: <select style={inputStyle} defaultValue={selected?.ticket_status} onChange={(e) => setTicketDetails({ ...ticketDetails, status: e.target.value })}>
                {statusList}
              </select>
              {/* Date de création du ticket */}
              Posté le:<input style={inputStyle} defaultValue={selected?.ticket_start_date} type="date" onChange={(e) => setTicketDetails({ ...ticketDetails, start_date: e.target.value })} />
              {/* Date de fin du ticket */}
              Terminé le:<input style={inputStyle} defaultValue={selected?.ticket_end_date} type="date" onChange={(e) => setTicketDetails({ ...ticketDetails, end_date: e.target.value })} />
            </Box>
