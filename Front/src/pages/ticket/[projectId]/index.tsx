        ...
        for (let project of userData.project) {
            if (project.id == new URL(window.location.href).pathname.split('/')[2]) {
                for (let ticket of project.ticket.ticket) {
                    ...
                    } else if (ticket.start_date < new Date().toISOString() && ticket.end_date < new Date().toISOString() && ticket.status === "fermé") {
                        updatedTickets.done.push(ticket);
                    } else if (ticket.start_date < new Date().toISOString() && ticket.end_date < new Date().toISOString() && ticket.status === "ouvert") {
                    ...
                    <div className='todo_container'>
                        ...
                        <div className='card_container'>
                            {projectTickets.todo.map((item: any) => (
                                <TicketCard key={item.key} id={item.key} title={item.title} start={item.start_date} end={item.end_date} urgence={item.urgenceId} updated_at={item.updated_at} />
                            ))}
                        </div>
