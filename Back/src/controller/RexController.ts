...
export class RexController {
   ...
   */
  public async createRex(@Body() data: Rex) {
    try {
  ...
  public async getOne(@Param("id") id: string) {
    try {
      const rex: Rex = await this.rexRepository.findOne({ where: { id } });
   ...
   */
  public async getAllRexByProject(@Param("projectid") projectid: string) {
    try {
   ...
   */
  public async update(@Param("id") id: string, @Body() data: Rex) {
    try {
      const rex: Rex = await this.rexRepository.findOne({ where: { id } });
      if (!rex) throw new Error("Rex not found");
