...
export class UserController {
   ...
   * @returns The user object if found, otherwise an error object.
   */
  public async getOneByMail(@Param("mail") mail: string) {
    try {
  ...
  public async update(
    @Param("id") id: string,
    @Body() data: User,
    @UploadedFiles("avatar", { options: multerConfig }) storedFiles: Array<any>
  ) {
    try {
      var user: User = await this.userRepository.findOne({ where: { id } });
      if (!user) throw new Error("Account not found");
